...
-- ── PR Configuration ──────────────────────────────────────────
-- ── PR module exports (upvalues — declared in main chunk before IIFE) ─────────
-- Only symbols referenced from outside the PR IIFE are listed here.
-- All other PR internals live inside the IIFE's own register space.
local PR_CFG, PR_Registry, PR_SchemaInfer, PR_FreqProfiler, PR_SeqAnalyzer
local PR_DepGraph, PR_Interceptor, PR_ActiveProber, PR_ManifestBuilder
local PR_Persist, PR_Bridge, PR_Analytics, PR_Classifier, PR_AnomalyDetector
local PR_EchoCalibrator, PR_ProtocolFingerprint, PR_PFP_KEY
...
-- ── PR Core State ─────────────────────────────────────────────
PR_Registry  = {}   -- [remoteName] = RemoteRecord
PR_SeqLog    = {}   -- ring buffer: {name, dir, t, argCount}
PR_SeqLogPtr = 0
PR_DepEdges  = {}   -- [fromName][toName] = {count, totalDelay, minDelay, maxDelay}
PR_Manifest  = {}
PR_Hooks_PR  = {}   -- connection cleanup list (named to avoid collision)
PR_LastProbe = 0
PR_Started   = false
...
PR_ANOMALY_ZSCORE_THRESH = 3.2   -- z-score threshold for spike detection
PR_ANOMALY_SILENCE_SEC   = 12.0  -- periodic remote silent > this → anomaly
...
PR_EchoCalibrator_Deltas     = {}   -- live delta samples from calibrator remote
PR_EchoCalibrator_Remote     = nil  -- currently watched remote record
PR_EchoCalibrator_Conn       = nil  -- event connection
PR_EchoCalibrator_LastT      = 0
PR_EchoCalibrator_MinSamples = 12
PR_EchoCalibrator_MaxSamples = 64
PR_EchoCalibrator_RefinedHz  = 0
PR_EchoCalibrator_Active     = false
...
PR_PFP_KEY        = "PR_Fingerprint_" .. tostring(game.PlaceId)
PR_CurrentPFP     = nil   -- {hash, remoteCount, roleMap, builtAt}
PR_LastPFP        = nil   -- loaded from _G on startup
